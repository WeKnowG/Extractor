...
def extract_items(lm_model, subject_model, object_model, tokenizer, text_in, id2rel, h_bar=0.1, t_bar=0.1):
...
#     print(sub_tails_logits)
    ...
    sub_heads, sub_tails = np.where(sub_heads_logits.cpu()[0] > h_bar)[0], np.where(sub_tails_logits.cpu()[0] > t_bar)[0]
    ...
    if subjects:
        triple_list = []
        token_ids = np.repeat(token_ids.cpu(), len(subjects), 0)
        segment_ids = np.repeat(segment_ids.cpu(), len(subjects), 0)
        attention_mask = np.repeat(attention_mask.cpu(), len(subjects), 0)
        
        sub_heads, sub_tails = np.array([sub[1:] for sub in subjects]).T.reshape((2, -1, 1))
        
        
        obj_heads_logits, obj_tails_logits = object_model(bert_outputs, torch.tensor(sub_heads).long().to(config.device).view(1, -1), torch.tensor(sub_tails).long().to(config.device).view(1, -1))
